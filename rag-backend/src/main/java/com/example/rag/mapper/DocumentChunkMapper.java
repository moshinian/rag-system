...
public interface DocumentChunkMapper extends BaseMapper<DocumentChunkEntity> {
    ...
    @Select({
            ...
            "           dc.embedding_model AS embeddingModel,",
            "           CAST((",
            "               CASE WHEN LOWER(dc.content) LIKE CONCAT('%', LOWER(#{questionPhrase}), '%') THEN #{phraseWeight} ELSE 0 END",
            "               + CASE WHEN LOWER(COALESCE(dc.title, '')) LIKE CONCAT('%', LOWER(#{questionPhrase}), '%') THEN #{phraseWeight} * #{titleWeight} ELSE 0 END",
            "               <foreach collection='terms' item='term'>",
            "                   + CASE WHEN LOWER(dc.content) LIKE CONCAT('%', LOWER(#{term}), '%')",
            "                         THEN 1.0 / GREATEST(1.0, LN(10 + dc.content_length))",
            "                         ELSE 0 END",
            "                   + CASE WHEN LOWER(COALESCE(dc.title, '')) LIKE CONCAT('%', LOWER(#{term}), '%') THEN #{titleWeight} ELSE 0 END",
            "               </foreach>",
            ...
            "           OR (LOWER(COALESCE(dc.title, '')) LIKE CONCAT('%', LOWER(#{questionPhrase}), '%'))",
            ...
            "                   OR LOWER(COALESCE(dc.title, '')) LIKE CONCAT('%', LOWER(#{term}), '%')",
            "               </foreach>",
            "           </if>)",
            ") scored",
            "WHERE scored.score &gt;= #{minHitThreshold}",
            "ORDER BY scored.score DESC, scored.chunkIndex ASC, scored.id ASC",
            "LIMIT #{limit}",
            "</script>"
    })
    List<RetrievedChunkCandidate> findTopKeywordChunksByLike(@Param("knowledgeBaseId") Long knowledgeBaseId,
                                                             @Param("questionPhrase") String questionPhrase,
                                                             @Param("terms") List<String> terms,
                                                             @Param("phraseWeight") double phraseWeight,
                                                             @Param("titleWeight") double titleWeight,
                                                             @Param("minHitThreshold") double minHitThreshold,
    ...
    @Select({
            ...
            "       dc.embedding_model AS embeddingModel,",
            "       CASE",
            "           WHEN #{rankFunction} = 'ts_rank' THEN ts_rank(",
            "               to_tsvector(CAST(#{tsConfig} AS regconfig), COALESCE(dc.title, '') || ' ' || dc.content),",
            "               websearch_to_tsquery(CAST(#{tsConfig} AS regconfig), #{queryText})",
            "           )",
            "           ELSE ts_rank_cd(",
            "               to_tsvector(CAST(#{tsConfig} AS regconfig), COALESCE(dc.title, '') || ' ' || dc.content),",
            "               websearch_to_tsquery(CAST(#{tsConfig} AS regconfig), #{queryText})",
            ...
            "  AND dc.embedding_vector IS NOT NULL",
            "  AND d.status = 'INDEXED'",
            "  AND to_tsvector(CAST(#{tsConfig} AS regconfig), COALESCE(dc.title, '') || ' ' || dc.content)",
            "      @@ websearch_to_tsquery(CAST(#{tsConfig} AS regconfig), #{queryText})",
            "ORDER BY score DESC, dc.chunk_index ASC, dc.id ASC",
            "LIMIT #{limit}",
            "</script>"
    })
    List<RetrievedChunkCandidate> findTopKeywordChunksByFts(@Param("knowledgeBaseId") Long knowledgeBaseId,
                                                            @Param("queryText") String queryText,
                                                            @Param("tsConfig") String tsConfig,
                                                            @Param("rankFunction") String rankFunction,
                                                            @Param("limit") int limit);
    @Update("""

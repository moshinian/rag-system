...
1. 前端基础工程、路由、全局 Provider、状态存储和统一 API Client 已完成。
2. 知识库、文档、检索、问答、历史和系统健康页面已形成第一版闭环。
3. `ApiResponse` 统一解包、错误提示、`requestId` 展示和常用轮询逻辑已完成收口。
4. 已完成与后端真实接口的一轮综合联调，覆盖健康、知识库、文档、异步索引、检索、问答和历史主链路。
5. 当前前端已经具备企业后台式 RAG 工作台雏形，下一步重点是继续补体验细节、验证链路和演示材料。

这个计划文档保留后续推进方向，当前真实状态以 [current-status.md](./current-status.md) 为准。

## 1. 目标

`rag-frontend` 下一阶段的目标不是继续堆页面数量，而是：

**把当前已经跑通的工作台，推进成“链路清晰、状态可解释、适合演示和面试展示”的稳定前端外壳。**

下一阶段要解决的核心问题包括：

1. 如何让知识库 -> 文档 -> 索引 -> 检索 -> 问答 -> 历史 的主用户流更顺滑。
2. 如何把后端的状态机、readiness 和错误语义翻译成清晰可操作的前端反馈。
3. 如何让前端页面不仅能操作系统，还能体现 RAG 系统的可解释性和工程完成度。

## 2. 当前定位

当前阶段，`rag-frontend` 的定位固定为：

**项目的 RAG 工作台前端。**

它负责的事情包括：

1. 组织知识库、文档、异步索引、检索、问答和历史的用户操作链路。
2. 承接 `qa/readiness`、任务状态、错误提示和来源展示等可视化表达。
3. 作为演示、联调和面试讲解时的系统操作外壳。

当前阶段它不负责的事情包括：

1. 重新发明后端状态机或私有业务语义。
2. 承接 provider 级模型治理和 AI Gateway 职责。
3. 为了页面数量而堆积和主链路无关的功能页。

## 3. 下一阶段重点

下一阶段优先继续收口这三件事：

1. 主链路体验：让上传、索引、检索、问答、历史回看更顺。
2. 状态与错误表达：让 readiness、任务阶段、失败原因和高级操作边界更清晰。
3. 演示与验证资产：补齐自动化、空态/异常态、开发说明和演示材料。

当前重点不是做“更多页面”，而是把已有页面做成真正可用、可讲透的工作台。
...
- 持续覆盖“创建知识库 -> 上传 -> 索引 -> 检索 -> 问答 -> 历史”的主点击链路。
- 验证 `qa/readiness`、索引任务轮询、错误提示和来源回看在正常/失败场景下都能正确表达。
- 确认页面状态文案继续基于后端真实字段和状态，不产生新的前端私有语义漂移。
- 继续通过构建产物、联调结果和浏览器级验证证明前端不是静态演示壳，而是真实可操作工作台。
